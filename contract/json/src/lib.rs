...
impl Interface for Contract1{
    ...
    fn ret_list(&mut self)->Vec<i32>{
        ...
        v[0] = v[1] + v[2];
        v.sort();
        let tar:Vec<i32> = v.into_iter().map(|f|f).collect();
        tar
    }
    fn save_str(&mut self,a:String)->bool{
    ...
    fn get_str(&mut self)->String{
        // let tar = storage_read(&STORAGE_KEY);
        // let v: Value = serde_json::from_str(&tar).unwrap();
        // v["name"].as_str().unwrap().into()
        let data = "{\"name\":\"John Doe\",\"age\":43,\"phones\":[\"+44 1234567\",\"+44 2345678\"]}";

        // Parse the string of data into serde_json::Value.
        let v: Value = serde_json::from_str(data).unwrap();
        let tar:String = v["name"].as_str().unwrap().into();
        tar        
    }
    fn ret_str(&mut self)->String{
        let s1: TinyStr4 = "tEsT".parse().unwrap();
        s1.to_ascii_uppercase().as_str().into()
    }
}
